...
def repo_checks(repo: Path) -> list[Check]:
    ...
    add(checks, "qbox_hexagon_command_queue_foundation", cmdq_foundation, "Apollo Hexagon DMA exposes command queue registers/faults, address overflow guarding, NOP/COPY/BARRIER/SIGNAL_FENCE execution, LOAD_EXECUTABLE metadata slots, executable-slot dispatch, and component-tested DISPATCH/VADD packet paths", "hexagon_apko_generic")
    model_kernel_stubs = (
        ...
        and has(dma_tests, r"CommandQueueDispatchesMnistLikeTransform")
    ...
    add(checks, "qbox_hexagon_model_kernel_stubs", model_kernel_stubs, "Apollo Hexagon DMA command queue has deterministic CNN and MNIST-like model-kernel stubs, including direct dispatch and executable-slot coverage", "hexagon_apko_generic")
    add(checks, "linux_driver_userspace_submit_abi", has(driver, r"DEFINE_DRM_ACCEL_FOPS") and has(driver, r"DRIVER_COMPUTE_ACCEL \| DRIVER_GEM") and has(driver, r"DRM_GEM_SHMEM_DRIVER_OPS") and has(driver, r"drm_dev_enter") and has(driver, r"drm_dev_unplug") and has(driver, r"APOLLO_HEXAGON_IOCTL\(SUBMIT_CNN") and has(driver, r"dynamic SMMU map refreshed"), "Apollo Linux driver exposes /dev/accel/accel* DRM accel submit ABI, initializes GEM SHMEM-backed default fops safely, and refreshes dynamic SMMU mappings", "hexagon_accelerator")
    ...
    add(checks, "linux_driver_bound_cmd_submit_vadd", has(driver, r"max_command_bytes = APOLLO_HEXAGON_CMDQ_SUBMIT_MAX_BYTES") and has(driver, r"max_bindings_per_dispatch = 2") and has(driver_bo, r"apollo_hexagon_context_copy_from_iova") and has(driver_bo, r"apollo_hexagon_context_get_iova_bo") and has(driver_bo, r"apollo_hexagon_bo_copy_to") and has(driver_exec, r"apollo_hexagon_prepare_bound_vadd") and has(driver_exec, r"apollo_hexagon_patch_bound_vadd") and has(driver_exec, r"apollo_hexagon_bound_vadd_put") and has(driver_exec, r"APOLLO_HEXAGON_CMDQ_OPCODE_LOAD_EXECUTABLE") and has(driver_exec, r"command BO LOAD_EXECUTABLE slot=") and has(driver_exec, r"command BO bound VADD dispatch") and has(driver_exec, r"command BO bound VADD output copied"), "Apollo Linux driver consumes BO_BIND metadata for the VADD CMD_SUBMIT path by scanning LOAD_EXECUTABLE metadata, copying input BO contents into the current QBox shared window, retaining an output GEM reference across the command wait, patching the command packet, and copying output back to the output BO", "hexagon_apko_generic")
...
def main() -> int:
    ...
    payload = {
        ...
        "classification": {
            ...
            "apollo_apko_generic_smoke": "generic_submit_slice_ready: APKO v0 metadata, Linux context, GEM BO lifecycle, BO binding metadata, VADD binding-table command BO submit, WAIT ioctls, invalid-IOVA GET_FAULT positive coverage, QBox command queue/fault registers, a QBox NOP/COPY/BARRIER/SIGNAL_FENCE packet execution subset, component-tested LOAD_EXECUTABLE metadata slots, executable-slot DISPATCH/VADD packet execution, deterministic QBox CNN/MNIST-like CMDQ model-kernel stubs, Linux APKO VADD CMDQ programming, executable create/destroy ioctls, legacy generic submit and fault retrieval ioctls, guest HAL APKO loader, staged VMFB-embedded APKO trailer loading, APKO VADD/CNN smoke scripts, and APKO negative ioctl smoke coverage exist; true hardware BO mapping, full APKO code execution, Linux/UMD CNN/MNIST CMDQ binding, and upstream VMFB HAL executable target backend packaging remain pending",

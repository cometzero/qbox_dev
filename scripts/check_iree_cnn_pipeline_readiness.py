...
def repo_checks(repo: Path) -> list[Check]:
    ...
    add(checks, "repo_hexagon_vector_add_uapi", has(hexagon_guest_uapi, r"APOLLO_HEXAGON_IOC_SUBMIT_VADD") and has(hexagon_hal, r"apollo_hexagon_queue_submit_vadd") and has(hexagon_runner, r"vector_add_graph") and has(hexagon_run_module, r"vector_add_graph"), "Apollo guest UAPI/HAL runner exposes a vector-add job path beside the tiny-CNN command path", "hexagon_vector_add")

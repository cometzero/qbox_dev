...
def repo_checks(repo: Path) -> list[Check]:
    ...
    add(checks, "qbox_hexagon_command_queue_foundation", cmdq_foundation, "Apollo Hexagon DMA exposes command queue registers/faults, address overflow guarding, NOP/COPY/BARRIER/SIGNAL_FENCE execution, LOAD_EXECUTABLE metadata slots, explicit LOAD_PAYLOAD records, separate LOAD_CODE APKO CODE model-dispatch entry validation, executable-slot dispatch, and component-tested DISPATCH/VADD packet paths", "hexagon_apko_generic")
    model_kernel_stubs = (
    ...
    add(checks, "linux_driver_cmdq_model_submit", linux_cmdq_model_submit, "Linux APKO VADD/CNN/MNIST generic submit can program a bounded command buffer with LOAD_EXECUTABLE, LOAD_PAYLOAD, separate LOAD_CODE model-dispatch entry validation, and DISPATCH; the command BO path consumes two BO bindings before ringing the CMDQ doorbell", "hexagon_apko_generic")

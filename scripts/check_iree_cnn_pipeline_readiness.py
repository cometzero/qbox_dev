...
def repo_checks(repo: Path) -> list[Check]:
    ...
    cmdq_foundation = (
        ...
        and has(dma_tests, r"CommandQueueLoadPayloadRejectsBadCodeEntry")
    ...
    add(checks, "qbox_hexagon_command_queue_foundation", cmdq_foundation, "Apollo Hexagon DMA exposes command queue registers/faults, address overflow guarding, NOP/COPY/BARRIER/SIGNAL_FENCE execution, LOAD_EXECUTABLE metadata slots, explicit LOAD_PAYLOAD records with decoded APKO CODE model-dispatch entry instructions, executable-slot dispatch, and component-tested DISPATCH/VADD packet paths", "hexagon_apko_generic")
    model_kernel_stubs = (

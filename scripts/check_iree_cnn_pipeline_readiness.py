...
def repo_checks(repo: Path) -> list[Check]:
    ...
    add(checks, "repo_apko_generic_smoke_scaffolds", apko_vector_add_hexagon_smoke_script.is_file() and os_access_executable(apko_vector_add_hexagon_smoke_script) and has(apko_vector_add_hexagon_smoke_script, r"generic APKO VADD smoke is blocked") and apko_tiny_cnn_hexagon_smoke_script.is_file() and os_access_executable(apko_tiny_cnn_hexagon_smoke_script) and has(apko_tiny_cnn_hexagon_smoke_script, r"generic APKO CNN smoke is blocked"), "repo-local APKO generic smoke scaffolds keep the compat vector-add and tiny-CNN lanes intact while making the current runtime ABI blocker explicit", "hexagon_apko_generic")
...
def main() -> int:
    ...
    payload = {
        ...
        "classification": {
            ...
            "hexagon_mlir_bridge": "adapter_slice: direct Hexagon-MLIR object execution is not feasible until the compiler toolchain and a generic QBox Hexagon loader ABI exist; vector-add can be bridged through existing ONNX->IREE artifacts plus the fixed Apollo VADD ioctl, with Hexagon-MLIR output staged as sidecar evidence",
            "apollo_apko_generic_smoke": "blocked_currently: APKO smoke scaffolds exist, but the Apollo Hexagon APKO executable/runtime ABI and generic command queue path are still missing; preserved compat CNN/VADD smoke remains the supported lane",

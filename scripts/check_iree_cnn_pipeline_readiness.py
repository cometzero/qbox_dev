...
def repo_checks(repo: Path) -> list[Check]:
    ...
    add(checks, "qbox_hexagon_command_queue_foundation", cmdq_foundation, "Apollo Hexagon DMA exposes command queue registers/faults, address overflow guarding, NOP/COPY/BARRIER/SIGNAL_FENCE execution, LOAD_EXECUTABLE metadata slots, explicit LOAD_PAYLOAD records, separate two-word LOAD_CODE APKO CODE program validation, executable-slot dispatch, and component-tested DISPATCH/VADD packet paths", "hexagon_apko_generic")
    model_kernel_stubs = (
    ...
    linux_cmdq_model_submit = (
        ...
        and has(driver_exec, r"apollo_hexagon_write_cmdq_dispatch_vadd")
        and has(driver_exec, r"apollo_hexagon_write_cmdq_dispatch_packet")
        and has(driver_exec, r"APOLLO_HEXAGON_REG_CMDQ_DOORBELL")
        and has(driver_exec, r"command BO LOAD_EXECUTABLE slot=")
        and has(driver_exec, r"command BO LOAD_PAYLOAD slot=")
        and has(driver_exec, r"command BO LOAD_CODE slot=")
        and has(driver_exec, r"payload_code_words")
        and has(driver_exec, r"payload_entry_word")
        and has(driver_exec, r"code_valid")
        and has(driver_exec, r"command BO bound %s dispatch")
    ...
    add(checks, "linux_driver_cmdq_model_submit", linux_cmdq_model_submit, "Linux APKO VADD/CNN/MNIST generic submit can program a bounded command buffer with LOAD_EXECUTABLE, LOAD_PAYLOAD, separate two-word LOAD_CODE program validation, and DISPATCH; the command BO path consumes two BO bindings before ringing the CMDQ doorbell", "hexagon_apko_generic")

...
external="${repo_root}/buildroot/external/apollo_qbox"
...
require_file "${board}/apollo_soc.dts" "Apollo QBox DTS template"
...
require_grep 'linux,initrd-start = <0x0 __INITRD_START__>' "${board}/apollo_soc.dts" "post-image initrd start placeholder"
require_grep 'compatible = "arm,cortex-a710"' "${board}/apollo_soc.dts" "A710 CPU compatible"
require_grep 'reserved-memory' "${board}/apollo_soc.dts" "future SRAM reservations"

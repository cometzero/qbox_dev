...
자동 생성했지만, 이제 APKO byte stream의 `PAYL` descriptor를 UMD가 읽어
`LOAD_PAYLOAD` packet으로 명시 전달한다.
...
```bash
bash -n scripts/*.sh
PYTHONPYCACHEPREFIX=/tmp/qbox-pycache-apko-payload-2 \
...
python3 scripts/check_iree_cnn_pipeline_readiness.py --repo . \
  --json build/verification/iree-readiness-payload-check.json
python3 scripts/check_apko_vmfb_verification_lane.py --repo . \
  --json build/verification/apko-vmfb-payload-check.json
...
- `LOAD_PAYLOAD`는 `PAYL` magic, version, descriptor word count, payload opcode를
  검증한다.
- QBox component test가 missing payload와 bad payload opcode를 malformed fault로
  확인한다.
- guest HAL은 APKO payload descriptor를 읽은 뒤 command BO에 `LOAD_PAYLOAD`를
  추가한다.
- guest HAL은 `PAYL` descriptor가 없는 기존 48-byte APKO v0 artifact는 legacy
  ...
  처리한다.
- VADD, CNN, MNIST staging artifact는 APKO header 뒤에 `PAYL` descriptor를 포함한다.

## 남은 작업

- APKO descriptor 뒤 실제 code/data section을 정의하고 실행기로 연결한다.
